...
var Form = function () {
    ...
    Form.prototype.register = function (vue, app) {
        ...
        vue.component('ark-switcher', {
            ...
            ready: function ready() {
                if (!this.active) this.active = this.$children[0].$el;
            }
        });
    ...
    Progress.prototype.register = function (Vue) {
        Vue.component('ark-progress', {
            ...
            ready: function ready() {
                var _this = this;
                if (this.live) {
                    var start_1 = Date.parse(this.live[0]),
                        end_1 = Date.parse(this.live[1]),
                        interval_1 = setInterval(function () {
                        var now = new Date().getTime(),
                            q = Math.abs(now - start_1),
                            d = Math.abs(end_1 - start_1),
                            percentage = q / d * 100;
                        if (percentage > 100) {
                            _this.data = 100;
